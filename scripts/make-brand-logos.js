/* =========================================================================
   ...
   사용: node scripts/make-brand-logos.js [원본폴더]
   - 원본(검정 로고 / 흰 배경 JPG·PNG) → 흰색 로고 / 투명 배경 WebP
   - 여백 트림 후 동일 캔버스(440x200)에 맞춰 크기·여백 통일
   - 새 브랜드 추가 시 아래 MAP 에 { id: '파일명' } 만 추가하면 됩니다
   ========================================================================= */
"use strict";
const sharp = require("sharp");
const path = require("path");

const SRC_DIR = process.argv[2] || "/Users/long/Downloads/골프 로고/";
...
const MAP = {
  ...
  malbon: "malbon.jpg",
  gfore: "gfore.jpg",
  titleist: "titleist.jpg",
  amazingcre: "amazingcre.jpg",
  anewgolf: "anewgolf.jpg",
};
...
(async () => {
  for (const [id, file] of Object.entries(MAP)) {
    ...
    // 1) 여백 트림 + 흑백 + 반전 (별도 패스여야 순서가 보장됨)
    const negated = await sharp(src).trim({ threshold: 20 }).greyscale().negate().png().toBuffer();
    // 2) 동일 박스에 맞추기 (검정 = 투명이 될 영역)
    ...
    const out = path.join(OUT_DIR, `brand-${id}.webp`);
    await sharp({ create: { width: W, height: H, channels: 3, background: { r: 255, g: 255, b: 255 } } })
      ...
      .toFile(out);
    console.log(`✅ brand-${id}.webp (${W}x${H})`);
  }
  console.log("\n※ 로고를 교체하면 db.js 의 logo 경로 뒤 ?v= 숫자를 올려 캐시를 무효화하세요.");
